...
(function ($, window, document, undefined) {
    ...
    var old = $.fn.clipPagination;

    // PROTOTYPE AND CONSTRUCTOR

    var clipPagination = function (element, options) {
        this.$element = $(element);
        this.options = $.extend({}, $.fn.clipPagination.defaults, options);
        this.init(this.options);
    };

    clipPagination.prototype = {

        constructor: clipPagination,
    ...
    $.fn.clipPagination = function (option) {
        var args = Array.prototype.slice.call(arguments, 1);
        var methodReturn;

        var $this = $(this);
        var data = $this.data('clip-pagination');
        var options = typeof option === 'object' && option;

        if (!data) $this.data('clip-pagination', (data = new clipPagination(this, options) ));
    ...
    $.fn.resetPaging = function () {
        $(this).data('clip-pagination', null);
    };

    $.fn.clipPagination.defaults = {
        totalPages: 0,
        startPage: 1,
        visiblePages: 5,
        ...
        hrefVariable: '{{number}}',
        first: 'First',
        prev: '<div class="icon-left"></div>',
        next: '<div class="icon-right"></div>',
        last: 'Last',
        paginationClass: 'pagination',
        onPageClick: null
    };

    $.fn.clipPagination.Constructor = clipPagination;

    $.fn.clipPagination.noConflict = function () {
        $.fn.clipPagination = old;
        return this;
    };
