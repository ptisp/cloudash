...
module.exports.validateUserResources = function(mem, cpu, hdd, cb, req) {
  ...
  cursor.each(function(err, doc) {
    ...
    if (doc !== null) {
      ...
      if (totalVMs + 1 <= parseInt(req.user.maxresources.vms) && totalMemory + (mem || 0) <= parseInt(req.user.maxresources.memory) && totalCPU + (cpu || 0) <= parseInt(req.user.maxresources.cpu) && totalDisk + (hdd || 0) <= parseInt(req.user.maxresources.storage)) {
        cb(undefined, true);
      } else {
        cb(undefined, false);
      }
