...
module.exports.validateUserResources = function(mem, cpu, hdd, cb, req) {
  ...
  cursor.each(function(err, doc) {
    ...
    if (doc !== null) {
      ...
      if (totalVMs + 1 <= req.user.maxresources.vms && totalMemory + (mem || 0) <= req.user.maxresources.memory && totalCPU + (cpu || 0) <= req.user.maxresources.cpu && totalDisk + (hdd || 0) <= req.user.maxresources.storage) {
        cb(undefined, true);
      } else {
        cb(undefined, false);
      }
