...
exports.createVm = function(req, res) {
  ...
  var insertToDB = function(id, cb){
    console.log(data.details);
    return vendors.mongo.collection('vms').insertOne({
      'type': 'opennebula',
      'providerId': id,
      'owner': req.user._id,
  ...
  if(req.user.type === 'admin' && data.openebulaid){
    if(isNaN(data.openebulaid)){
      return res.status(500).json({
        'error': 'Invalid Openebula ID'
      });
    }
    return insertToDB(parseInt(data.openebulaid,10), function(err, result){
  ...
  util.validateGlobalResources(data.details.ram, data.details.vcpu, data.details.disk, function(err, available) {
    ...
    if (req.user.auth.ssh && req.user.auth.ssh.length > 0) {
      ...
      context += 'CONTEXT=[NETWORK="YES", SSH_PUBLIC_KEY="' + req.user.auth.ssh + '"]\n';
    }
    ...
    template.instantiate(data.details.hostname, undefined, 'MEMORY="' + (data.details.ram || 1024) + '"\nVCPU="' + (data.details.vcpu || 1) + '"\n ' + context, function(err, vm) {
      ...
      insertToDB(vm.id, function(err, result){
        if (err) return res.status(500).json({
          'error': 'Error creating VM (DB)'
        });
        return res.status(200).json({
...
exports.deleteVm = function(req, res) {
  ...
  getVM(req, vmID, function(err, doc) {
    if (err || doc === null) {
      return res.status(500).json({
        'error': 'Error getting VM details'
      });
    }
