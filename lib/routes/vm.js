      ...
      request.get({
        ...
        request.post({
          ...
          return resp.status(200).json({
            ...
            'address': config.onesunstone.replace('https://', '').replace('http://', '').split(':')[0]
          });
        });
      });
    });
...
exports.interfaceAdd = function(req, res) {
  ...
  getVM(req, vmID, function(err, doc) {
    ...
    var vm = vendors.one.getVM(doc.providerId);
    vm.attachNIC('NIC=[NETWORK="' + config.onenetwork + '",NETWORK_UNAME="oneadmin"]', function(err, data) {
      if (err) {
        return res.status(500).json({
...
exports.interfaceDel = function(req, res) {
  ...
  getVM(req, vmID, function(err, doc) {
    var vm = vendors.one.getVM(doc.providerId);
    vm.detachNIC(interfaceID, function(err, data) {
      if (err) {
        return res.status(500).json({
